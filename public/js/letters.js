...
Letters.prototype = {
  init: function(options) {
    ...
    this.weight =
      options && options.hasOwnProperty('weight') ? options.weight : 1;
    this.rounded =
      options && options.hasOwnProperty('rounded') ? options.rounded : false;
    this.color =
      options && options.hasOwnProperty('color') ? options.color : '#5F6062';
    this.duration =
      options && options.hasOwnProperty('duration') ? options.duration : 1;
    this.delay =
      options && options.hasOwnProperty('delay') ? options.delay : [0, 0.05];
    this.fade = options && options.hasOwnProperty('fade') ? options.fade : 0.5;
    this.easing =
      options && options.hasOwnProperty('easing')
        ? options.easing
        : d3_ease.easeCubicInOut.ease;
    this.individualDelays =
      options && options.hasOwnProperty('individualDelays')
        ? options.individualDelays
        : false;
    this.visible = false;
  ...
  drawText: function() {
    ...
    var rounded =
      typeof self.rounded === 'boolean' ? self.rounded : self.rounded[0];
    var color = typeof self.color === 'string' ? self.color : self.color[0];
    var fade = typeof self.fade === 'number' ? self.fade : self.fade[0];
    self.renderAlphabet =
      typeof self.weight === 'number' && typeof self.rounded === 'boolean'
        ? self.initAlphabet(weight, rounded) && false
        : true;
    var spaces = 0;
    this.letters.forEach(function(element, index) {
      size = typeof self.size[index] === 'number' ? self.size[index] : size;
      weight =
        typeof self.weight[index] === 'number' ? self.weight[index] : weight;
      rounded =
        typeof self.rounded[index] === 'boolean'
          ? self.rounded[index]
          : rounded;
      color =
        typeof self.color !== 'string' && typeof self.color[index] === 'string'
          ? self.color[index]
          : color;
      ...
      if (element === ' ') {
        ...
        self.segments[index - spaces] = self.drawLetter(
          element,
          index - spaces,
          size,
          weight,
  ...
  show: function(options) {
    ...
    var _duration =
      options && options.hasOwnProperty('duration')
        ...
        : this.duration;
    var _delay =
      options && options.hasOwnProperty('delay') ? options.delay : this.delay;
    var _fade =
      options && options.hasOwnProperty('fade') ? options.fade : this.fade;
    var _easing =
      options && options.hasOwnProperty('easing')
        ? options.easing
        : this.easing;
    var _individualDelays =
      options && options.hasOwnProperty('individualDelays')
        ? options.individualDelays
        : this.individualDelays;
    var tempDelay = 0;
    ...
    this.letters.forEach(function(element, index) {
      lastDuration =
        typeof _duration[index] === 'number' ? _duration[index] : lastDuration;
      lastFade = typeof _fade[index] === 'number' ? _fade[index] : lastFade;
      lastEasing =
        typeof _easing[index] === 'function' ? _easing[index] : lastEasing;
      if (typeof _delay === 'number') {
        tempDelay = _individualDelays ? _delay : tempDelay + _delay;
      } else {
        lastDelay =
          typeof _delay[index] === 'number' ? _delay[index] : lastDelay;
      ...
      var _callback =
        index === length - 1 && options && options.hasOwnProperty('callback')
          ? options.callback
          : null;
  ...
  initAlphabet: function(weight, rounded) {
    ...
    var circleRight =
      'm ' + (width / 2 + 2 * radius) + ' ' + height / 2 + ' ' + circle;
    var circleF = 'm ' + width / 2 + ' ' + (height / 2 - 12) + ' ' + circle;
    var circleF2 =
      'm ' + width / 2 + ' ' + (height / 2 + weight / 2) + ' ' + circle;
    var circleGY =
      'm ' + width / 2 + ' ' + (height / 2 + radius + _weight) + ' ' + circle;
    var circleJ =
      'm ' +
      (width / 2 - radius - _weight) +
      ' ' +
      (height / 2 + radius + _weight) +
      ' ' +
      circle;
    var circleSXZ =
      'm ' + (width / 2 - radius - _weight) + ' ' + height / 2 + ' ' + circle;
    var circleSXZ2 =
      'm ' + (width / 2 + radius - _weight) + ' ' + height / 2 + ' ' + circle;
    ...
    this.alphabet = {
      ...
      b: {
        ...
        final: [
          { begin: '-125%', end: '-50%' },
          { begin: p50minus35, end: p50plus23 }
        ]
      },
      c: {
        svg: svgSmall,
      ...
      f: {
        ...
        initial: [
          { begin: '-25%', end: '-25%' },
          { begin: '1', end: '1' },
          { begin: '50% - 13', end: '50% - 13' }
        ],
        final: [
          { begin: '-25%', end: '1' },
          { begin: '-25%', end: '1' },
          { begin: '50% - 13', end: p50plus23 }
        ]
      },
      g: {
        svg: svgSmall,
      ...
      i: {
        ...
        initial: [{ begin: '50%', end: '50%' }, { begin: '50%', end: '50%' }],
        final: [
      ...
      m: {
        svg: svgLarge,
        paths: [circleLeft, circleRight, lineLeftLarge, lineRightLarge],
        initial: [
          { begin: '90%', end: '90%' },
          { begin: '90%', end: '90%' },
          { begin: '90%', end: '90%' },
          { begin: p50plus23, end: p50plus23 }
        ],
        final: [
          { begin: '-25%', end: '25%' },
          { begin: '-25%', end: '25%' },
      ...
      t: {
        ...
        final: [
          { begin: '50%', end: '75% + 1' },
          { begin: p50minus35, end: '50%' },
          { begin: p50minus20, end: '50%' }
        ]
      },
      u: {
        svg: svgSmall,
        paths: [circleCenter, lineRight, lineLeft],
        initial: [
          { begin: '50%', end: '50%' },
          { begin: '80%', end: '80%' },
          { begin: '50%', end: '50%' }
      ...
      y: {
        svg: svgSmall,
        paths: [circleSXZ2, circleGY, lineRight],
        initial: [
          { begin: '75%', end: '75%' },
          { begin: '1', end: '1' },
          { begin: p50minus23, end: p50minus23 }
